...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (ac == 0) continue;
        if (strcmp(av[0], "run") == 0) { system("./langmgr/run.sh demo.bas"); continue; }
        if (strcmp(av[0], "files") == 0) { system("./apps/files/file-manager.sh browse ."); continue; }
        if (strcmp(av[0], "launch") == 0) { system("./desktop/launcher-grid/launcher-grid.sh"); continue; }
        if (is_builtin(av[0])) {
